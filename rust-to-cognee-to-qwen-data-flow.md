...
Rust can call this through one of three approaches:

| Approach | How |
|---|---|
| Outbox | Rust writes `osai_cognee_outbox`; Python Cognee worker consumes it. |
| HTTP service | Small Python FastAPI service exposes `/remember` and `/recall`; Rust calls it. |
| Direct future SDK | Use a Rust SDK only if stable and production-ready. |

Best starting approach:

```text
Rust -> PostgreSQL outbox -> Python Cognee ingestion worker
```

This keeps the Rust agent clean and avoids mixing OS-level control with Python AI dependencies.
...
4. Rust creates compact memory text.
5. Rust writes compact memory to `osai_cognee_outbox`.
6. Python Cognee worker reads outbox and calls `cognee.remember()`.
7. Rust exposes `/api/ask`.
8. `/api/ask` fetches latest PostgreSQL facts.
9. `/api/ask` calls Cognee recall worker/API.
